function surf = surfOperators5d(surf)
...
%     trgArea(i) = 1/2*norm(n);
    ...
    grad5(i,[p1 p2 p3]) = [shapegrad1(5),shapegrad2(5),shapegrad3(5)];
...
%---- grad, div
surf.grad = @(u) [grad1*u,grad2*u,grad3*u,grad4*u,grad5*u];
surf.div = @(V) -( grad1'*(trgArea.*V(:,1)) ...
                  +grad2'*(trgArea.*V(:,2)) ...
                  +grad3'*(trgArea.*V(:,3)) ...
                  +grad4'*(trgArea.*V(:,4)) ...
                  +grad5'*(trgArea.*V(:,5)) );
surf.gradT = @(u) full(cat(3,(grad1*u),(grad2*u),(grad3*u),(grad4*u),(grad5*u)));
