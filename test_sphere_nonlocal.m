...
meshName = 'sphere';
[surf.pt,surf.trg] = readOFF(['../Data/',meshName,'.off']);
% meshName = 'euc_right_reg48';
% load(['../../Data/',meshName]);
...
%% kernel
cosdistmat = surf.pt*surf.pt';
distmat = sparse(nPt,nPt);
ind = cosdistmat>0.99;sum(ind(:));
distmat(ind) = acos(cosdistmat(ind));
mu = 1;
sigmasq = 0.01;
kernel = sparse(nPt,nPt);
kernel(ind) = mu*exp(-distmat(ind).^2/sigmasq);
...
lambdaF = 5e1;
% egName = 'nonlocal';
% opts.funcF = @(rho) lambdaF/2*rho.*(kernel*(surf.ptArea.*rho));
% opts.gradF = @(rho) lambdaF*kernel*(surf.ptArea.*rho);
lambdaG = 5e-1;
